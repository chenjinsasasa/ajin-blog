...
- Required frontmatter for every post:
  ...
  - `excerpt`
  - `author`
- For all newly created posts, `coverImage` and the Image 2 provenance fields are required before commit/push.
...
- When creating or updating a post:
  1. Fill in the article title, excerpt, Image 2 provenance fields, and a `.png` `coverImage` path.
  2. Run `npm run cover:image2:generate -- --post <post-path>`.
  3. Visually compare the result with all four locked references; reject anything outside the steam industrial age engraving world.
  4. Run `npm run cover:image2:validate -- --post <post-path>`.
  5. Run `npm run verify` before commit/push.
